...
# Support MRs generated by LocalRegistry.jl, which contain HTML elements and no newlines
# See https://github.com/invenia/TagBotGitLab/issues/35
good_body_with_html = (
    "• Registering package: Example<br>"
    "• Repository: gitlab.foo.com/foo/bar<br>"
    "• Version: v0.1.2<br>"
    "• Commit: abcdef<br>"
...
def test_parse_body():
    ...
    assert err is None
    repo, version, commit, err = tagbot.parse_body(good_body_with_html)
    assert repo == "foo/bar"
    assert version == "v0.1.2"
    assert commit == "abcdef"
    assert err is None
